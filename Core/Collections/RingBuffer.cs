...
namespace Telefrek.Core.Collections
{
    ...
    public class RingBuffer<T> : IRingBuffer<T>
    {
        ...
        bool _TryRemove(out T[] items, int minItems, int maxItems, TimeSpan timeout)
        {
            items = new T[0];

            lock (_syncLock)
            {
                while (!_isClosed && Size < minItems)
                    if (!Monitor.Wait(_syncLock, timeout))
                        return false; // Failed to synchronize in time

                // Can't return a new value from a closed buffer with no publishing
                if (_isClosed && Size < minItems)
                    return false;

                // Resize the array
                var l = Size > maxItems ? maxItems : Size;
                items = new T[l];
        ...
        public bool IsComplete { get { return _isClosed && _size == 0; } }
        ...
        public bool TryRemoveRange(out T[] instances)
        {
            ...
            return _TryRemove(out instances, 1, int.MaxValue, MAX_WAIT);
        }
        ...
        public bool TryRemoveRange(out T[] instances, TimeSpan timeout)
        {
            ...
            return _TryRemove(out instances, 1, int.MaxValue, timeout);
        }
        ...
        public bool TryRemoveRange(out T[] instances, int maxItems, TimeSpan timeout)
        {
            ...
            return _TryRemove(out instances, 1, maxItems, timeout);
        }

        public bool TryRemoveRange(out T[] instances, int minItems, int maxItems, TimeSpan timeout)
        {
            if (!_isOpen)
                throw new InvalidOperationException("Buffer must be opened before reading");
            if (timeout < TimeSpan.Zero)
            ...
            return _TryRemove(out instances, minItems, maxItems, timeout);
        }
